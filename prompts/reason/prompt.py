...
from prompts.reason.schema import JsonSchema
from langchain.prompts import PromptTemplate
...
def get_templatechatgpt(memory: List[Episode] = None, Dicts={}):
    ...
    template = BASE_TEMPLATE.format(
        related_knowledge=Dicts["related_knowledge"],
        related_past_episodes=Dicts["related_past_episodes"],
        task=Dicts["task"],
        tool_info=Dicts["tool_info"],
    )
    return template
