...
from prompts.extract_entity.schema import JsonSchema
from prompts.extract_entity.prompt import get_template, get_chat_template
from langchain_openai import OpenAIEmbeddings
from prompts.extract_entity.schema import JsonSchema as ENTITY_EXTRACTION_SCHEMA
from prompts.json_output_parser import LLMJsonOutputParser, LLMJsonOutputParserException
import openai
...
class SemanticMemory:
    def __init__(self, num_episodes: int = 0, embeddings: Optional[OpenAIEmbeddings] = None, vector_store: Optional[VectorStore] = None):
    ...
    def extract_entity(self, text: str) -> dict:
        ...
        results = self.gpt.chat_with_local_ollama(system_prompt = schema, prompt = prompt)
        print(f"entity extraction: {results['response']}")
        entity_extraction_result = str(results['response'])

        # Parse and validate the result
        try:
            result_json_obj = LLMJsonOutputParser.parse_and_validate(
                json_str=entity_extraction_result, json_schema=CREATE_JSON_SCHEMA_STR
            )
