...
from agent.task_verification import TaskVerifier
...
class BootstrapAgent:
    def __init__(self, dir: str = DEFAULT_AGENT_DIR, agent_name: str = DEFAULT_AGENT_NAME, gpt: Optional[ChatGPT] = None, procedural_memory: ProceduralMemory = ProceduralMemory(), episodic_memory: Optional[EpisodicMemory] = None, semantic_memory: Optional[SemanticMemory] = None, task_manager: Optional[TaskManager] = None, working_memory: Optional[deque] = deque(maxlen=12), thoughts: Optional[List[Any]] = [], messages: Optional[List[Any]] = [], last_task: Optional[str] = None) -> None:
        ...
        self.task_verifier = TaskVerifier(api_key=OPENAI_API_KEY)
    ...
    def continuous_learning_loop(self):
        while True:
            ...
            while incomplete_tasks:
                ...
                incomplete_tasks = self.task_manager.get_incomplete_tasks()
                if self.verify_task_completion(action, action_result):
    ...
    def execute_action(self, task_description: str) -> None:
        ...
        action_plan = self.gpt.chat_with_local_ollama(
            system_prompt=ReasonPrompt.SCHEMA_TEMPLATE,
            prompt=ReasonPrompt.BASE_TEMPLATE.format(**prompt_data)
        )
        action_result = self.process_action_plan(action_plan)
        return action_plan.get('response'), action_result

    def process_action_plan(self, action_plan: Dict[str, Any]) -> None:
        """Process the action plan generated by the LLM."""
        try:
            ...
            action = action_plan.get('response')
            if not action:
                print("No 'response' key found in the action plan.")
                return

            # If action is a string, parse it as JSON
            if isinstance(action, str):
                try:
                    # Remove \n characters from the 'action' string
            ...
            if act is None:
                print("No 'action' key found in the response.")
                return
            ...
            # Extract the tool_name and args from the act dictionary
            tool_name = act.get('tool_name')
            args = act.get('args', {}) # Default to an empty dictionary if 'args' is not present
            
            action_result = self._act(tool_name, args)
            return action_result
        except KeyError as e:
            print(f"Missing key in action plan: {e}")
        except TypeError as e:
            print(f"Error processing action plan: {e}")

    def evaluate_and_learn(self, action: str, action_result: str) -> None:
        """Evaluate the action taken and the result, then learn from it."""
        try:
            prompt_data = {
                "action": action,
    ...
    def verify_task_completion(self, action: str, action_result: str) -> bool:
        return self.task_verifier.verify_task_completion(action, action_result)
